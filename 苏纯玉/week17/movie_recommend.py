...
def find_similar_item(user_to_rating):
    ...
    for user, user_rating in user_to_rating.items():
        for movie_id, score in enumerate(user_rating):
            if movie_id + 1 not in item_to_vector:
                item_to_vector[movie_id + 1] = []
            item_to_vector[movie_id + 1].append(score)

    for item_a, vector_a in item_to_vector.items():
        similar_items[item_a] = []
        for item_b, vector_b in item_to_vector.items():
            if item_a == item_b :
                continue
            if "%d_%d"%(item_b, item_a) in score_buffer:
                similarity = score_buffer["%d_%d"%(item_b, item_a)]
            else:
                similarity = cosine_distance(np.array(vector_a), np.array(vector_b))
                score_buffer["%d_%d" % (item_a, item_b)] = similarity
            similar_items[item_a].append([item_b, similarity])
        similar_items[item_a] = sorted(similar_items[item_a], reverse=True, key=lambda x: x[1])
...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    try:
        ...
        for similar_item, similarity in similar_items[item_id][:topn]:
            rating_by_similar_item = user_to_rating[user_id][similar_item]
